...
Scores are checked by replaying. A new run asks `POST /api/runs` for a run id and derives its seed from it; at the end, `POST /api/scope-creep/finish` sends the actions, and the Worker (`src/scopecreep.ts`) replays them with the same engine file. If every action is legal and the run has ended, it records the replayed score. Replaying a whole run takes under a millisecond. Run ids last a fortnight, since runs are long and can be resumed.
